...
async function analyzeImage(base64,mediaType,slotId,lang="en") {
  ...
  try {
    const res=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({base64,mediaType,lang})});
...
function MealModal({ slot, entry, onSave, onClose, isBackfill, S, t, userId, lang }) {
  ...
  const doAIAnalysis=async()=>{
    ...
    try {
      const analysis=await analyzeImage(snapshot.base64,snapshot.mediaType,slot.id,lang);
      setAnalyzing(false);
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:200}} onClick={onClose}>
      <div style={{background:S.card,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:560,maxHeight:"90vh",overflow:"auto",padding:"18px 18px 36px"}} onClick={e=>e.stopPropagation()}>
        ...
        {pendingPreset&&!analyzing&&<div style={{marginBottom:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <button onClick={e=>{e.stopPropagation();doAIAnalysis();}} style={{background:S.accentDim,border:`1px solid ${S.accentBorder}`,color:S.accent,borderRadius:11,padding:"12px 0",fontSize:13,fontWeight:700,cursor:"pointer"}}>{lang==="es"?"🤖 Analizar con IA":"🤖 Analyze with AI"}</button>
            <button onClick={()=>setShowPresets(p=>!p)} style={{background:showPresets?"rgba(255,180,80,0.12)":S.surface,border:`1px solid ${showPresets?"rgba(255,180,80,0.3)":S.border}`,color:showPresets?"#ffb450":S.textMuted,borderRadius:11,padding:"12px 0",fontSize:13,fontWeight:600,cursor:"pointer"}}>{lang==="es"?"⭐ Usar Preset":"⭐ Use Preset"}</button>
          </div>
  ...
  useEffect(()=>{
    ...
    const checkReminders=()=>{
      ...
      Object.entries(saved).forEach(([slotId,cfg])=>{
        ...
        const slot=getMealSlots("en").find(s=>s.id===slotId);
        new Notification("Tracky. 🥑",{body:`Don't forget to log your ${slot?.label||slotId}!`,icon:"/favicon.ico"});
      });
    };
    checkReminders();
