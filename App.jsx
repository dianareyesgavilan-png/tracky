...
const T = {
  en: {
    ...
    presetName:"Preset name…", noPresets:"No presets yet", use:"Use",
  },
  es: {
    ...
    presetName:"Nombre del preset…", noPresets:"Sin presets todavía", use:"Usar",
  },
};
  ...
  return (
    <div style={{marginTop:10,display:"flex",gap:7}}>
      ...
      <button onClick={doSave} style={{background:"rgba(255,180,80,0.1)",border:"1px solid rgba(255,180,80,0.3)",color:saved?"#7ecf7a":"#ffb450",borderRadius:9,padding:"7px 12px",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0,transition:"color 0.3s"}}>
        {saved?(lang==="es"?"✓ Guardado":"✓ Saved"):(lang==="es"?"⭐ Guardar Preset":"⭐ Save as Preset")}
      </button>
    </div>
  );
}

function IngredientEditor({ photo, onConfirm, onCancel, S, t, userId, slotId }) {
...
function MealModal({ slot, entry, onSave, onClose, isBackfill, S, t, userId, lang }) {
  ...
  return (
    <>{pendingPhoto&&<IngredientEditor photo={pendingPhoto} onConfirm={handleConfirm} onCancel={handleCancel} S={S} t={t} userId={userId} slotId={slot.id}/>}
