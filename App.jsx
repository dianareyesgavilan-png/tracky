...
async function analyzeImage(base64,mediaType,slotId) {
  ...
  try {
    const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:1000,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:mediaType,data:base64}},{type:"text",text:`Analyze this meal. Return ONLY valid JSON:\n{"description":"one sentence","ingredients":[{"name":"item with portion","calories":0,"protein":0,"carbs":0,"fat":0,"fibre":0}],"totals":{"calories":0,"protein":0,"carbs":0,"fat":0,"fibre":0}}`}]}]})});
    const data=await res.json(); const txt=data.content.map(i=>i.text||"").join("");
    return JSON.parse(txt.replace(/```json|```/g,"").trim());
  } catch(e){ return{description:"",ingredients:[],totals:{calories:0,protein:0,carbs:0,fat:0,fibre:0}}; }
}
...
function IngredientEditor({ photo, onConfirm, onCancel, S, t }) {
  ...
  const totals=ingredients.reduce((a,i)=>({calories:a.calories+(i.calories||0),protein:a.protein+(i.protein||0),carbs:a.carbs+(i.carbs||0),fat:a.fat+(i.fat||0),fibre:a.fibre+(i.fibre||0)}),{calories:0,protein:0,carbs:0,fat:0,fibre:0});
  const removeItem=i=>setIngredients(p=>p.filter((_,j)=>j!==i));
  const startEdit=i=>{setEditIdx(i);setEditVal({...ingredients[i]});};
  const commitEdit=()=>{setIngredients(p=>p.map((v,i)=>i===editIdx?{...v,...editVal}:v));setEditIdx(null);};
  const addItem=()=>{if(!newName.trim())return;setIngredients(p=>[...p,{name:newName.trim(),calories:0,protein:0,carbs:0,fat:0,fibre:0}]);setNewName("");inputRef.current?.focus();};
  ...
  const NumInput=({label,field,color})=>(
    <div style={{textAlign:"center"}}>
      <input type="number" min="0" value={editVal[field]||0} onChange={e=>setEditVal(v=>({...v,[field]:parseInt(e.target.value)||0}))}
      ...
      <div style={{background:S.card,borderRadius:20,width:"100%",maxWidth:500,maxHeight:"96vh",overflow:"auto",border:`1px solid ${S.accentBorder}`,boxShadow:"0 24px 64px rgba(0,0,0,0.7)"}}>
        ...
        <div style={{padding:"14px 16px 22px"}}>
          ...
          <div style={{display:"flex",gap:7,marginBottom:16}}>
            <input ref={inputRef} value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addItem();}}} placeholder={t.addIngredient}
              ...
              onFocus={e=>e.target.style.borderColor=S.accentBorder} onBlur={e=>e.target.style.borderColor=S.border}/>
            <button onClick={addItem} style={{background:S.accentDim,border:`1px solid ${S.accentBorder}`,color:S.accent,borderRadius:9,padding:"8px 14px",fontSize:16,cursor:"pointer",fontWeight:700}}>+</button>
          </div>
          <div style={{display:"flex",gap:8}}>
...
function AuthScreen({ onLogin }) {
  ...
  if(loading) return <div style={{minHeight:"100vh",background:"#0b120b",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{color:"rgba(255,255,255,0.3)",fontSize:13}}>Loading…</div></div>;
