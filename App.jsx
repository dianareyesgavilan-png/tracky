...
function MealModal({ slot, entry, onSave, onClose, isBackfill, S, t, userId }) {
  ...
  const doAIAnalysis=async()=>{
    ...
    const analysis=await analyzeImage(pendingPreset.base64,pendingPreset.mediaType,slot.id);
    setAnalyzing(false);
...
function MealGrid({ meals, onUpdate, readOnly, isBackfill, S, t, lang, userId }) {
  ...
  return (
    ...
    {activeSlot&&<MealModal slot={activeSlot} entry={meals?.[activeSlot.id]} onSave={entry=>onUpdate(activeSlot.id,entry)} onClose={()=>setActiveSlot(null)} isBackfill={isBackfill} S={S} t={t} userId={userId}/>}
    </>
  );
}
