...
function SettingsModal({ user, S, lang, skinId, onSave, onClose }) {
  ...
  const requestAndSave=async()=>{
    ...
    setNotifStatus(perm);
    if(perm==="granted"){localStorage.setItem(`tracky-reminders-${user.id}`,JSON.stringify(reminders));alert(t.reminderSet);}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:400,padding:16}} onClick={onClose}>
      <div style={{background:S.card,borderRadius:20,width:"100%",maxWidth:380,maxHeight:"92vh",overflow:"auto",padding:"22px 20px 26px",border:`1px solid ${S.border}`}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>{localStorage.setItem(`tracky-reminders-${user.id}`,JSON.stringify(reminders));onSave(selLang,selSkin);}} style={{flex:1,background:PS.accent,color:PS.bg,border:"none",borderRadius:11,padding:"12px 0",fontSize:13,fontWeight:700,cursor:"pointer"}}>{t.save}</button>
...
function App({ user, onLogout }) {
  ...
  useEffect(()=>{(async()=>{
    ...
    // Auto-remind to backup weekly
    const lastBackup=localStorage.getItem(`tracky-lastbackup-${user.id}`);
    const daysSince=lastBackup?(Date.now()-parseInt(lastBackup))/(1000*60*60*24):999;
    if(daysSince>7) {
      setTimeout(()=>{
        if(window.confirm(lang==="es"?"Han pasado más de 7 días sin backup. ¿Exportar ahora?":"It's been 7+ days without a backup. Export now?")) {
  ...
  useEffect(()=>{
    ...
    const checkReminders=()=>{
      const saved=JSON.parse(localStorage.getItem(`tracky-reminders-${user.id}`)||"null");
      if(!saved) return;
      const todayMeals=loadUserData(user.id)[todayKey()]?.meals||{};
      if(navigator.serviceWorker?.controller){
        navigator.serviceWorker.controller.postMessage({type:"CHECK_REMINDERS",reminders:saved,loggedMeals:todayMeals,lang});
      } else {
        const now=new Date(); const hhmm=`${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}`;
        const slots=getMealSlots(lang);
        Object.entries(saved).forEach(([slotId,cfg])=>{
          if(!cfg.enabled||cfg.time!==hhmm) return;
          if(todayMeals[slotId]?.status==="done") return;
          const slot=slots.find(s=>s.id===slotId);
          new Notification("Tracky. 🥑",{body:lang==="es"?`No olvides registrar tu ${slot?.label||slotId}!`:`Don't forget to log your ${slot?.label||slotId}!`});
        });
      }
    };
    checkReminders();
