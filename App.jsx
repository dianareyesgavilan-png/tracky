...
function MealModal({ slot, entry, onSave, onClose, isBackfill, S, t, userId, lang }) {
  ...
  const doAIAnalysis=async()=>{
    ...
    const analysis=await analyzeImage(pendingPreset.base64,pendingPreset.mediaType,slot.id,lang);
    setAnalyzing(false);
    await new Promise(resolve=>setPendingPhoto({...pendingPreset,...analysis,_resolve:resolve}));
  };
