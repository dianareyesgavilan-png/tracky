...
function MealModal({ slot, entry, onSave, onClose, isBackfill, S, t, userId, lang }) {
  ...
  const editPhoto=(i)=>{ const p=photos[i]; setEditingPhotoIdx(i); new Promise(resolve=>setPendingPhoto({...p,imageUrl:p.base64?`data:${p.mediaType||"image/jpeg"};base64,${p.base64}`:null,_resolve:resolve})); };
...
function App({ user, onLogout }) {
  ...
  const persist=useCallback((date,nd)=>{ setHistory(prev=>{ const nh={...prev,[date]:nd}; saveUserData(user.id,nh); return nh; }); },[user.id]);
