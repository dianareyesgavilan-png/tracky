...
function MealModal({ slot, entry, onSave, onClose, isBackfill, S, t, userId, lang }) {
  ...
  const doAIAnalysis=async()=>{
    ...
    try {
      ...
      setAnalyzing(false);
      if(!analysis||!analysis.ingredients){alert("AI error: no ingredients returned. Check Vercel logs.");return;}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:200}} onClick={onClose}>
      <div style={{background:S.card,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:560,maxHeight:"90vh",overflow:"auto",padding:"18px 18px 36px"}} onClick={e=>e.stopPropagation()}>
        ...
        {pendingPreset&&!analyzing&&<div style={{marginBottom:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <button onClick={e=>{e.stopPropagation();doAIAnalysis();}} style={{background:S.accentDim,border:`1px solid ${S.accentBorder}`,color:S.accent,borderRadius:11,padding:"12px 0",fontSize:13,fontWeight:700,cursor:"pointer"}}>🤖 Analyze with AI</button>
            <button onClick={()=>setShowPresets(p=>!p)} style={{background:showPresets?"rgba(255,180,80,0.12)":S.surface,border:`1px solid ${showPresets?"rgba(255,180,80,0.3)":S.border}`,color:showPresets?"#ffb450":S.textMuted,borderRadius:11,padding:"12px 0",fontSize:13,fontWeight:600,cursor:"pointer"}}>⭐ Use Preset</button>
          </div>
            ...
            {presets.filter(p=>p.slotId===slot.id).length===0
              ...
              :presets.filter(p=>p.slotId===slot.id).map((p,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderBottom:`1px solid ${S.border}`}}>
                  ...
                  <button onClick={()=>usePresetEntry(p)} style={{background:S.accentDim,border:`1px solid ${S.accentBorder}`,color:S.accent,borderRadius:8,padding:"5px 10px",fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0}}>Use</button>
