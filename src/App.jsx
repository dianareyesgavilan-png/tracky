...
const T = {
  en: {
    ...
    confirmSave:"✓ Confirm & Save", description:"Description", mealTitle:"Meal title (optional)", ingredients:"Ingredients",
    ...
    demoTag:"Demo mode — AI is mocked", thisWeek:"This Week", weightTrend:"Weight trend", weeklyAvg:"Weekly average", activity:"Activity", totalMins:"total mins", proteinAvg:"Protein", carbsAvg:"Carbs", fatAvg:"Fat",
    ...
    reminders:"Reminders", remindersHint:"Notify me if I haven't logged by:", enableReminders:"Enable Reminders",
    ...
    pdfExport:"PDF ↗", backup:"Backup", restoreBackup:"Restore backup", backupOk:"✓ Restored!", backupErr:"Invalid file", noFoods:"No foods logged", daysTap:"Tap any day to view or edit",
  ...
  es: {
    ...
    confirmSave:"✓ Confirmar y guardar", description:"Descripción", mealTitle:"Título de la comida (opcional)", ingredients:"Ingredientes",
    ...
    demoTag:"Modo demo — IA simulada", thisWeek:"Esta Semana", weightTrend:"Tendencia de peso", weeklyAvg:"Promedio semanal", activity:"Actividad", totalMins:"min totales", proteinAvg:"Proteína", carbsAvg:"Carbs", fatAvg:"Grasa",
    ...
    reminders:"Recordatorios", remindersHint:"Avisarme si no registré a las:", enableReminders:"Activar Recordatorios",
    ...
    pdfExport:"PDF ↗", backup:"Backup", restoreBackup:"Restaurar backup", backupOk:"✓ Restaurado!", backupErr:"Archivo inválido", noFoods:"Sin comidas registradas", daysTap:"Toca cualquier día para ver o editar",
...
/* ═══ CONSTANTS ═══════════════════════════════════════════════════════════ */
const IS_DEMO = false;
const WORKOUT_TYPES = ["Run 🏃","Walk 🚶","Cycle 🚴","Swim 🏊","Gym 🏋️","Tennis 🎾","Yoga 🧘","HIIT ⚡","Pilates 🩰","Hike 🥾","Other 💪"];
...
async function analyzeImage(base64,mediaType,slotId,lang="en",title="") {
  ...
  try {
    const res=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({base64,mediaType,lang,title})});
...
function SavePresetBar({ description, ingredients, totals, slotId, userId, S, t }) {
  ...
  return (
    <div style={{marginTop:10,display:"flex",gap:7}}>
      <input value={name} onChange={e=>setName(e.target.value)} placeholder={t.presetName}
        style={{flex:1,background:"rgba(255,255,255,0.04)",border:`1px solid ${S.border}`,borderRadius:9,padding:"7px 10px",color:S.text,fontSize:12,outline:"none",fontFamily:"'DM Sans',sans-serif"}}
        onFocus={e=>e.target.style.borderColor=S.accentBorder} onBlur={e=>e.target.style.borderColor=S.border}/>
      <button onClick={doSave} style={{background:"rgba(255,180,80,0.1)",border:"1px solid rgba(255,180,80,0.3)",color:saved?"#7ecf7a":"#ffb450",borderRadius:9,padding:"7px 12px",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0,transition:"color 0.3s"}}>
        {saved?(t.presetSaved||"✓ Saved"):("⭐ "+(t.saveAsPreset||"Save as Preset"))}
      </button>
    </div>
  );
}
...
function IngredientEditor({ photo, onConfirm, onCancel, S, t, userId, slotId, lang="en" }) {
  ...
  const parsedEditVal=editIdx!==null?{...editVal,calories:parseInt(editVal.calories)||0,protein:parseInt(editVal.protein)||0,carbs:parseInt(editVal.carbs)||0,fat:parseInt(editVal.fat)||0,fibre:parseInt(editVal.fibre)||0}:{};
  const liveIngredients=editIdx!==null?ingredients.map((v,i)=>i===editIdx?{...v,...parsedEditVal}:v):ingredients;
  const totals=liveIngredients.reduce((a,i)=>({calories:a.calories+(parseInt(i.calories)||0),protein:a.protein+(parseInt(i.protein)||0),carbs:a.carbs+(parseInt(i.carbs)||0),fat:a.fat+(parseInt(i.fat)||0),fibre:a.fibre+(parseInt(i.fibre)||0)}),{calories:0,protein:0,carbs:0,fat:0,fibre:0});
  ...
  const addItem=async()=>{
    ...
    try {
      const res=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:name,lang})});
      const data=await res.json();
      const raw=data.ingredients?.[0]||{name,calories:0,protein:0,carbs:0,fat:0,fibre:0};
      const newIng={name,calories:parseInt(raw.calories)||0,protein:parseInt(raw.protein)||0,carbs:parseInt(raw.carbs)||0,fat:parseInt(raw.fat)||0,fibre:parseInt(raw.fibre)||0};
      setIngredients(p=>{ setEditIdx(p.length); setEditVal(newIng); return [...p,newIng]; });
    } catch(e){
      const newIng={name,calories:0,protein:0,carbs:0,fat:0,fibre:0};
      setIngredients(p=>{ setEditIdx(p.length); setEditVal(newIng); return [...p,newIng]; });
  ...
  const NumInput=({label,field,color})=>(
    <div style={{textAlign:"center"}}>
      <input type="number" min="0" value={editVal[field]===undefined?"":editVal[field]} onChange={e=>setEditVal(v=>({...v,[field]:e.target.value}))} onBlur={e=>setEditVal(v=>({...v,[field]:parseInt(e.target.value)||0}))}
      ...
      <div style={{background:S.card,borderRadius:20,width:"100%",maxWidth:500,maxHeight:"96vh",overflow:"auto",border:`1px solid ${S.accentBorder}`,boxShadow:"0 24px 64px rgba(0,0,0,0.7)"}}>
        ...
        <div style={{padding:"14px 16px 22px"}}>
          ...
          <div style={{marginBottom:10}}>
            <div style={{fontSize:10,color:S.textMuted,textTransform:"uppercase",letterSpacing:1,marginBottom:5}}>{t.mealTitle}</div>
            <input value={title} onChange={e=>setTitle(e.target.value)} placeholder={description.slice(0,40)||"e.g. Almuerzo en casa"}
              style={{width:"100%",background:"rgba(255,255,255,0.04)",border:`1px solid ${S.border}`,borderRadius:9,padding:"9px 11px",color:S.text,fontSize:13,fontFamily:"'DM Sans',sans-serif",outline:"none"}}
              onFocus={e=>e.target.style.borderColor=S.accentBorder} onBlur={e=>e.target.style.borderColor=S.border}/>
          </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {ingredients.map((ing,i)=>editIdx===i?(
                <div key={i} style={{background:S.accentDim,border:`1px solid ${S.accentBorder}`,borderRadius:10,padding:"10px 12px"}}>
                  <div style={{fontSize:13,fontWeight:600,color:S.accent,marginBottom:8,padding:"4px 2px"}}>{editVal.name}</div>
...
function MealModal({ slot, entry, onSave, onClose, isBackfill, S, t, userId, lang }) {
  ...
  const [pendingPreset,setPendingPreset]=useState(null); // photo waiting for AI or preset choice
  ...
  const handleFiles=async(files)=>{ 
    ...
    for(const file of arr){ 
      const rawB64=await toBase64(file);
      const resized=await resizeImage(rawB64,file.type);
      const imageUrl=URL.createObjectURL(file);
      const data={imageUrl,base64:resized,mediaType:"image/jpeg"};
      pendingPresetRef.current=data;
      setPendingPreset(data);
    }
  };

  const doAIAnalysis=async(title="")=>{
    const snapshot=pendingPresetRef.current;
    if(!snapshot) { alert("No photo found"); return; }
    pendingPresetRef.current=null;
    setShowTitleInput(false); setTitleInput("");
    setAnalyzing(true); setPendingPreset(null);
    try {
      const analysis=await analyzeImage(snapshot.base64,snapshot.mediaType,slot.id,lang,title);
      setAnalyzing(false);
      if(!analysis){alert("AI error: no response");return;}
      await new Promise(resolve=>setPendingPhoto({...snapshot,...analysis,_resolve:resolve}));
    } catch(e){
      setAnalyzing(false);
  ...
  const doManualEntry=async()=>{
    ...
    pendingPresetRef.current=null;
    setShowManualInput(false); setManualInput(""); setPendingPreset(null);
    setAnalyzing(true);
    try {
      const res=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:name,lang})});
      const data=await res.json();
      const ing={name,calories:parseInt(data.ingredients?.[0]?.calories)||0,protein:parseInt(data.ingredients?.[0]?.protein)||0,carbs:parseInt(data.ingredients?.[0]?.carbs)||0,fat:parseInt(data.ingredients?.[0]?.fat)||0,fibre:parseInt(data.ingredients?.[0]?.fibre)||0};
      const totals=ing;
      setAnalyzing(false);
      await new Promise(resolve=>setPendingPhoto({...snapshot,title:name,description:name,ingredients:[ing],totals,_resolve:resolve}));
    } catch(e){
      setAnalyzing(false);
      await new Promise(resolve=>setPendingPhoto({...snapshot,title:name,description:name,ingredients:[{name,calories:0,protein:0,carbs:0,fat:0,fibre:0}],totals:{calories:0,protein:0,carbs:0,fat:0,fibre:0},_resolve:resolve}));
    }
  };
  const usePresetEntry=(preset)=>{
    if(!pendingPreset) return;
    setPhotos(p=>[...p,{base64:pendingPreset.base64,mediaType:"image/jpeg",description:preset.description,ingredients:preset.ingredients,totals:preset.totals}]);
    setPendingPreset(null); setShowPresets(false);
  };
      ...
      <div style={{background:S.card,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:560,maxHeight:"90vh",overflow:"auto",padding:"18px 18px 36px"}} onClick={e=>e.stopPropagation()}>
        ...
        {previews.length>0&&<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
          {previews.map((p,i)=><div key={i} style={{position:"relative",borderRadius:10,overflow:"hidden",aspectRatio:"1"}}>
            ...
            <button onClick={e=>{e.stopPropagation();editPhoto(i);}} style={{position:"absolute",top:4,left:4,background:"rgba(0,0,0,0.65)",border:"none",color:S.accent,borderRadius:"50%",width:20,height:20,fontSize:10,cursor:"pointer",lineHeight:"20px",textAlign:"center",padding:0}}>✏️</button>
            <label onClick={e=>e.stopPropagation()} style={{position:"absolute",bottom:26,left:4,background:"rgba(0,0,0,0.65)",borderRadius:"50%",width:20,height:20,fontSize:10,cursor:"pointer",lineHeight:"20px",textAlign:"center",display:"block",color:"#fff"}}>
              📷<input type="file" accept="image/*" style={{display:"none"}} onChange={e=>{const f=e.target.files[0];if(f)replacePhotoImage(i,f);e.target.value="";}}/>
            </label>
        ...
        {/* Upload area — no photos yet and no pending */}
        {previews.length===0&&!pendingPreset&&<div onClick={()=>!analyzing&&fileRef.current.click()} style={{border:`1.5px dashed ${S.border}`,borderRadius:14,padding:"24px 0",textAlign:"center",cursor:analyzing?"default":"pointer",marginBottom:12,color:S.textMuted}}>
          {analyzing
            ?<><div style={{fontSize:12,color:S.accent,display:"flex",alignItems:"center",justifyContent:"center",gap:7,marginBottom:5}}><div style={{width:7,height:7,borderRadius:"50%",background:S.accent,animation:"blink 1s infinite"}}/>{t.analyzing}…</div><div style={{fontSize:11,opacity:0.5}}>{t.analyzeHint}</div></>
            :<><div style={{fontSize:26,marginBottom:6}}>📷</div><div style={{fontSize:13}}>{t.addPhotos}</div></>}
        </div>}

        {/* Photo pending — show AI or Preset choice */}
        {pendingPreset&&!analyzing&&<div style={{marginBottom:12}}>
          <div style={{position:"relative",borderRadius:12,overflow:"hidden",aspectRatio:"16/9",marginBottom:10}}>
            <img src={pendingPreset.imageUrl} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            <button onClick={()=>setPendingPreset(null)} style={{position:"absolute",top:6,right:6,background:"rgba(0,0,0,0.65)",border:"none",color:"#fff",borderRadius:"50%",width:22,height:22,fontSize:13,cursor:"pointer",lineHeight:"22px",textAlign:"center",padding:0}}>×</button>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:5}}>
            <button onClick={e=>{e.stopPropagation();setShowTitleInput(false);setShowPresets(false);doAIAnalysis("");}} style={{background:S.accentDim,border:`1px solid ${S.accentBorder}`,color:S.accent,borderRadius:11,padding:"10px 0",fontSize:11,fontWeight:700,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><span>🤖</span><span>IA</span></button>
            <button onClick={()=>{setShowTitleInput(p=>!p);setShowPresets(false);}} style={{background:showTitleInput?"rgba(120,200,255,0.12)":S.surface,border:`1px solid ${showTitleInput?"rgba(120,200,255,0.3)":S.border}`,color:showTitleInput?"#7ec8ff":S.textMuted,borderRadius:11,padding:"10px 0",fontSize:11,fontWeight:600,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><span>📝</span><span>{lang==="es"?"Título":"Title"}</span></button>
            <button onClick={()=>{setShowManualInput(p=>!p);setShowTitleInput(false);setShowPresets(false);}} style={{background:showManualInput?"rgba(180,255,180,0.1)":S.surface,border:`1px solid ${showManualInput?"rgba(180,255,180,0.3)":S.border}`,color:showManualInput?"#7ecf7a":S.textMuted,borderRadius:11,padding:"10px 0",fontSize:11,fontWeight:600,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><span>✍️</span><span>{lang==="es"?"Manual":"Manual"}</span></button>
            <button onClick={()=>{setShowPresets(p=>!p);setShowTitleInput(false);}} style={{background:showPresets?"rgba(255,180,80,0.12)":S.surface,border:`1px solid ${showPresets?"rgba(255,180,80,0.3)":S.border}`,color:showPresets?"#ffb450":S.textMuted,borderRadius:11,padding:"10px 0",fontSize:11,fontWeight:600,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><span>⭐</span><span>Preset</span></button>
          </div>
          {showManualInput&&<div style={{marginTop:8,display:"flex",gap:8}}>
            <input autoFocus value={manualInput} onChange={e=>setManualInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doManualEntry()} placeholder={lang==="es"?"ej. manzana, arroz con leche...":"e.g. apple, pasta..."}
              style={{flex:1,background:"rgba(255,255,255,0.05)",border:`1px solid rgba(180,255,180,0.3)`,borderRadius:10,padding:"9px 12px",color:S.text,fontSize:13,fontFamily:"'DM Sans',sans-serif",outline:"none"}}/>
            <button onClick={doManualEntry} style={{background:"#7ecf7a",border:"none",color:"#111",borderRadius:10,padding:"9px 14px",fontSize:13,fontWeight:700,cursor:"pointer",flexShrink:0}}>{lang==="es"?"Buscar":"Search"}</button>
          </div>}
          {showTitleInput&&<div style={{marginTop:8,display:"flex",gap:8}}>
            <input autoFocus value={titleInput} onChange={e=>setTitleInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&titleInput.trim()&&doAIAnalysis(titleInput.trim())} placeholder={lang==="es"?"ej. milanesa con puré":"e.g. grilled chicken with rice"}
              style={{flex:1,background:"rgba(255,255,255,0.05)",border:`1px solid ${S.accentBorder}`,borderRadius:10,padding:"9px 12px",color:S.text,fontSize:13,fontFamily:"'DM Sans',sans-serif",outline:"none"}}/>
        ...
        {analyzing&&<div style={{fontSize:12,color:S.accent,marginBottom:10,display:"flex",alignItems:"center",gap:7}}><div style={{width:7,height:7,borderRadius:"50%",background:S.accent,animation:"blink 1s infinite"}}/>{t.analyzing}…</div>}
...
/* ═══ MEAL GRID ══════════════════════════════════════════════════════════ */
function MealGrid({ meals, onUpdate, readOnly, isBackfill, S, t, lang, userId }) {
  ...
  return (
    ...
    {activeSlot&&<MealModal slot={activeSlot} entry={meals?.[activeSlot.id]} onSave={entry=>onUpdate(activeSlot.id,entry)} onClose={()=>setActiveSlot(null)} isBackfill={isBackfill} S={S} t={t} userId={userId} lang={lang}/>}
    </>
  );
}
...
function WeekView({ history, S, t, lang }) {
  ...
  return (
    <div style={{animation:"fadeUp 0.35s ease"}}>
      ...
      {(()=>{
        const daysWithData=week.filter(d=>Object.values(history[d]?.meals||{}).some(e=>e?.status==="done"));
        if(!daysWithData.length) return null;
        const totals=daysWithData.reduce((acc,d)=>{
          Object.values(history[d]?.meals||{}).forEach(e=>{
            if(e?.status==="done"&&e.totals){acc.kcal+=(e.totals.calories||0);acc.protein+=(e.totals.protein||0);acc.carbs+=(e.totals.carbs||0);acc.fat+=(e.totals.fat||0);}
          });
          return acc;
        },{kcal:0,protein:0,carbs:0,fat:0});
        const n=daysWithData.length;
        const avg={kcal:Math.round(totals.kcal/n),protein:Math.round(totals.protein/n),carbs:Math.round(totals.carbs/n),fat:Math.round(totals.fat/n)};
        return <div style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:13,padding:"14px 12px",marginBottom:12}}>
          <div style={{fontSize:10,color:S.textMuted,textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>{t.weeklyAvg}</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,textAlign:"center"}}>
            {[[avg.kcal,"kcal",S.accent],[avg.protein+"g",t.proteinAvg,S.blue],[avg.carbs+"g",t.carbsAvg,S.orange],[avg.fat+"g",t.fatAvg,"#c084fc"]].map(([val,lbl,col])=>
              <div key={lbl}><div style={{fontSize:16,fontWeight:700,color:col,fontFamily:"monospace"}}>{val}</div><div style={{fontSize:9,color:S.textMuted,marginTop:2,textTransform:"uppercase",letterSpacing:0.5}}>{lbl}</div></div>
            )}
          </div>
        </div>;
      })()}
      {/* Activity summary */}
      {(()=>{
        const workoutDays=week.filter(d=>history[d]?.workout?.type);
        if(!workoutDays.length) return null;
        const totalMins=workoutDays.reduce((s,d)=>s+(parseInt(history[d].workout.duration)||0),0);
        const types=[...new Set(workoutDays.map(d=>history[d].workout.type))];
        return <div style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:13,padding:"14px 12px",marginBottom:12}}>
          <div style={{fontSize:10,color:S.textMuted,textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>{t.activity}</div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div style={{fontSize:22,fontWeight:700,color:S.orange,fontFamily:"monospace"}}>{workoutDays.length}<span style={{fontSize:11,color:S.textMuted,fontWeight:400}}> {t.days}</span></div>
            {totalMins>0&&<div style={{fontSize:13,color:S.textMuted}}>{totalMins} {t.totalMins}</div>}
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
            {types.map(tp=><span key={tp} style={{background:"rgba(255,180,80,0.1)",border:"1px solid rgba(255,180,80,0.2)",borderRadius:20,padding:"3px 10px",fontSize:11,color:S.orange}}>{tp}</span>)}
          </div>
        </div>;
      })()}
  ...
  return (
    <div style={{display:"grid",gap:8,width:"100%"}}>
      {entries.map(([date,day])=>{
        ...
        const slots=getMealSlots(lang);
        const title=slots.map(s=>day.meals?.[s.id]?.photos?.[0]?.title).filter(Boolean)[0];
        const foodNames=slots.flatMap(s=>(day.meals?.[s.id]?.ingredients||[]).map(i=>i.name)).filter(Boolean).slice(0,3);
        const desc=title||foodNames.slice(0,2).join(", ")||slots.map(s=>day.meals?.[s.id]?.foods?.[0]).filter(Boolean).slice(0,2).join(", ");
        return <button key={date} onClick={()=>onSelect(date)} style={{background:S.surface,border:`1px solid ${S.border}`,borderRadius:13,padding:"12px 14px",cursor:"pointer",textAlign:"left",color:S.text,display:"flex",alignItems:"center",gap:12,width:"100%",transition:"background 0.15s"}} onMouseOver={e=>e.currentTarget.style.background=S.accentDim} onMouseOut={e=>e.currentTarget.style.background=S.surface}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{display:"flex",gap:7,marginTop:3}}>
              ...
              {day.workout?.type&&<span style={{fontSize:10,color:S.orange,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:80}}>🏋️ {day.workout.type.slice(0,12)}</span>}
...
/* ═══ BACKUP ══════════════════════════════════════════════════════════════ */
function exportBackup(history, todayData, today, userId, userName) {
  const all = {...history, [today]: todayData};
  const blob = new Blob([JSON.stringify({version:1, userId, userName, exportedAt: new Date().toISOString(), data: all}, null, 2)], {type:'application/json'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `tracky-backup-${new Date().toISOString().slice(0,10)}.json`;
  a.click();
...
function App({ user, onLogout }) {
  ...
  useEffect(()=>{
    ...
    const saved=JSON.parse(localStorage.getItem(`tracky-reminders-${user.id}`)||"null");
    if(!saved) return;
    const sendToSW=(sw)=>{ sw.postMessage({type:"SCHEDULE_REMINDERS",reminders:saved,lang}); };
            ...
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              ...
              <label title={t.restoreBackup} style={{background:S.surface,border:`1px solid ${S.border}`,color:S.textMuted,borderRadius:9,padding:"5px 9px",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center"}}>
                ...
                <input type="file" accept=".json" style={{display:"none"}} onChange={e=>{
                  ...
                  importBackup(f, user.id, data=>{ setHistory(data); if(data[today])setTodayData(data[today]); setBackupMsg(t.backupOk); setTimeout(()=>setBackupMsg(""),3000); }, err=>{ setBackupMsg(t.backupErr); setTimeout(()=>setBackupMsg(""),3000); });
                  e.target.value="";
                }}/>
              </label>
